-- Let a still-unprovisioned signup accept an employee invitation. This covers
-- Auth providers that preserve the verified email but omit redirect query
-- parameters or retain an early default owner profile.
...
begin
  ...
  if existing_profile.id is not null
    and existing_profile.account_type <> 'employee'
    and not (
      existing_profile.account_status = 'pending_email'
      and not exists (
        select 1 from public.business_members as owned_membership
        where owned_membership.user_id = p_user_id
          and owned_membership.role = 'owner'
          and owned_membership.status = 'active'
      )
    ) then
