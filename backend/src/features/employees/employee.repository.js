...
export const employeeRepository = {
  listMemberships(businessId) { return rows(getSupabaseAdmin().from("business_members").select("user_id,role,status,joined_at").eq("business_id", businessId).eq("role", "employee").neq("status", "removed").order("joined_at", { ascending: false })); },
  listProfiles(ids) { return ids.length ? rows(getSupabaseAdmin().from("profiles").select("id,email,full_name,account_status").in("id", ids)) : []; },
  listInvitations(businessId) { return rows(getSupabaseAdmin().from("employee_invitations").select("id,email,status,expires_at,created_at,updated_at").eq("business_id", businessId).order("created_at", { ascending: false })); },
  async findPendingInvitation(businessId, email) { const { data, error } = await getSupabaseAdmin().from("employee_invitations").select("*").eq("business_id", businessId).ilike("email", email).eq("status", "pending").maybeSingle(); throwDatabaseError(error); return data; },
  async findInvitation(businessId, invitationId) { const { data, error } = await getSupabaseAdmin().from("employee_invitations").select("*").eq("business_id", businessId).eq("id", invitationId).maybeSingle(); throwDatabaseError(error); return data; },
