...
export const employeeRepository = {
  ...
  async findProfileByEmail(email) { const { data, error } = await getSupabaseAdmin().from("profiles").select("id").eq("email", email.toLowerCase()).maybeSingle(); throwDatabaseError(error); return data; },
  findPendingInvitationsByEmail(email) { return rows(getSupabaseAdmin().from("employee_invitations").select("id,token_hash").eq("email", email.toLowerCase()).eq("status", "pending").gt("expires_at", new Date().toISOString()).order("created_at", { ascending: false }).limit(2)); },
