...
export async function removeUser(request, response, next) { try { await platformService.removeUser({ actorUserId: request.auth.user.id, ...request.validated }); return sendSuccess(response, { message: "User permanently deleted" }); } catch (error) { return next(error); } }
