...
test("unconfirmed owner is blocked distinctly from pending approval", async () => { const error = await run(requireApprovedOwner, auth({ profile: { account_status: "pending_email" }, businessStatus: "pending_approval" })); assert.equal(error.code, "EMAIL_CONFIRMATION_REQUIRED"); });
test("rejected owner remains rejected and cannot be treated as pending", async () => { const access = auth({ profile: { account_status: "rejected" }, businessStatus: "rejected" }); const result = getAccessState(access); const error = await run(requireApprovedOwner, access); assert.equal(result.state, "rejected"); assert.notEqual(error.code, "PENDING_APPROVAL"); });
